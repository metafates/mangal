...
func (a *AnilistClient) SavePreferences() error {
	...
	if exists, err := afero.Exists(filesystem.Get(), filepath.Dir(anilistFile)); err != nil {
		return err
	} else if !exists {
		if err := filesystem.Get().MkdirAll(filepath.Dir(anilistFile), 0777); err != nil {
			return err
		}
	}
	...
	if exists, err := afero.Exists(filesystem.Get(), anilistFile); err != nil {
		...
		if err := afero.WriteFile(filesystem.Get(), anilistFile, jsonPreferences, 0777); err != nil {
			return err
		}
...
`
	...
	for i, media := range response.Data.Page.Media {
		urls[i] = &AnilistURL{
			ID:      media.ID,
			Address: "https://anilist.co/manga/" + util.ToString(media.ID),
			Title:   util.IfElse(media.Title.English != "", media.Title.English, media.Title.Romaji),
		}
	}
